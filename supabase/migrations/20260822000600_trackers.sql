...
-- su animal. Por eso los pings de esta tabla no se comparten NUNCA con otros
...
create index tracker_devices_pet_idx on public.tracker_devices (pet_id);
...
create index tracker_pings_pet_time_idx on public.tracker_pings (pet_id, recorded_at desc);

-- ---------------------------------------------------------------------------
-- Geocercas: lo que dispara el check-in automático.
--
-- El tutor no tiene que acordarse de nada. El animal llega al parque, el radar se
...
create index geofences_pet_idx on public.geofences (pet_id) where active;

/**
 * Purga los pings antiguos.
 *
 * Conserva el último punto de cada dispositivo —hace falta para saber dónde
 * estaba el animal la última vez— y borra el resto pasados treinta días. La
